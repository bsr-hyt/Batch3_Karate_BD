Feature: basic api tests
  ...
  #run da en altta response kısmını, onun bir üstünde de response header var.
  #target - surefine-reports - karate-summary de report var.
  @book
  ...
  Scenario: basic header verification with bookstore
    ...
    #verify if work of "Date" exist in the response body(Response da header da Date diye bir keyword var mı?)
  ...
  Scenario: basic json body verification with bookstore
    ...
    #verify that third subtitle is exist
